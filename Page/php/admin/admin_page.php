<?php
include '../user/config.php'; // chỉ include header, không query DB nữa
session_start();
...
async function loadStats() {
    try {
        ...
        container.innerHTML = `
            ${createStatCard('fas fa-clock text-warning', 'Tổng tiền chờ xử lý', new Intl.NumberFormat('vi-VN').format(data.total_pendings) + ' VNĐ', 'bg-warning bg-opacity-10', 'Tổng tiền chờ xử lý')}
            ${createStatCard('fas fa-check-circle text-success', 'Tổng tiền đã thanh toán', new Intl.NumberFormat('vi-VN').format(data.total_completed) + ' VNĐ', 'bg-success bg-opacity-10', 'Tổng tiền đã thanh toán')}
